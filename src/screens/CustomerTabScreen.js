...
export default function ReceiptHistoryScreen({ navigation }) {
  ...
  const load = async () => {
    const data = await getReceiptSnapshots();
    setSnapshots(data);
  };
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            await deleteReceiptSnapshot(item.id);
            load();
          },
        },
      ]
              ...
              <TouchableOpacity onPress={() => handleView(item)} activeOpacity={0.8}>
                <View style={styles.cardRow}>
                  ...
                  <View style={styles.cardRight}>
                    <Text style={styles.totalBill}>{formatCurrency(item.total_bill)}</Text>
                    <View style={[
                      styles.statusBadge,
                      { backgroundColor: item.balance <= 0 ? Colors.successLight : Colors.warningLight }
                    ]}>
                      <Text style={[
                        styles.statusText,
                        { color: item.balance <= 0 ? Colors.success : Colors.warning }
                      ]}>
...
const styles = StyleSheet.create({
  ...
  totalBill: { fontSize: 16, fontWeight: '800', color: Colors.primary },
