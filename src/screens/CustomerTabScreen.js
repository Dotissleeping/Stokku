...
import {
  ...
  removeTabItem, saveReceiptSnapshot,
} from '../database/db';
          ...
          <ScrollView style={styles.productList} showsVerticalScrollIndicator={false}>
            ...
            {filtered.map((p) => (
              ...
              >
                ...
                {selected?.id === p.id && <Text style={[styles.checkmark, { color: Colors.primary }]}>✓</Text>}
              </TouchableOpacity>
            ))}
          </ScrollView>
          {selected && (
...
export default function CustomerTabScreen({ route, navigation }) {
  ...
  const handleRemoveItem = (item) => {
    Alert.alert('Remove Item', `Remove ${item.product_name} from this tab?`, [
      ...
      { text: 'Remove', style: 'destructive', onPress: async () => { await removeTabItem(item.id, item.product_id, item.quantity); load(); } },
    ]);
  };
