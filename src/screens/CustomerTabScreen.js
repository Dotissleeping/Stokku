...
export default function CustomerTabScreen({ route, navigation }) {
  ...
  const handlePayment = async (amount) => {
    ...
    await saveReceiptSnapshot(
      initCustomer.id,
      customer?.name || initCustomer.name,
      items, pays, bill, paid, bal
    );
    load();
  };
              ...
              {payments.map((pay, index) => (
                <View key={pay.id}>
                  ...
                  <View style={styles.paymentItem}>
                    ...
                    <TouchableOpacity onPress={() => handleDeletePayment(pay)} style={styles.undoBtn}>
...
const styles = StyleSheet.create({
  ...
  undoBtn: { paddingHorizontal: 12, paddingVertical: 6, backgroundColor: Colors.dangerLight, borderRadius: Radius.full },
  ...
  paymentInput: {
    ...
    textAlign: 'center', borderWidth: 2, borderColor: Colors.primary, marginBottom: Spacing.md,
  },
