    ...
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
        <View style={[styles.modalSheet, { maxHeight: 360 }]}>
          ...
          {product && (
            <Text style={styles.restockProductName}>{product.name}</Text>
          )}
          <Input
          ...
          products.length > 0 ? (
            <Animated.View entering={FadeInDown.duration(300)} style={styles.listHeader}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('RestockHistory')}>
                <Text style={styles.historyLink}>Restock History →</Text>
              </TouchableOpacity>
            </Animated.View>
          ) : null
        }
...
const styles = StyleSheet.create({
  ...
  restockProductName: { fontSize: 15, color: Colors.textSecondary, marginBottom: Spacing.md, fontWeight: '600' },
